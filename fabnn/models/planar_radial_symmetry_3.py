...
def make_symmetrical_block(feature_shape, params: Dict):
    ...
    bias_initializer = get_initializer(params["quadrant_bias_initializer"])
    kernel_initializer = get_initializer(params["quadrant_kernel_initializer"])
...
def make_model(params: Dict):
    ...
    for i in range(1, levels):
        ...
        sym_block = make_symmetrical_block(symmetrical_block_shape)
        r1 = sym_block(q1)
        r2 = sym_block(q2)
        r3 = sym_block(q3)
        r4 = sym_block(q4)
    ...
    sym_end_block = make_symmetrical_end_block(quadrant_size)
    quadrants = []
    for idx in range(4):
        quadrant = Lambda(lambda x: x[:, quadrant_size * idx : quadrant_size * (idx + 1)])(
            prev_layer
        )
        quadrants.append(
            Reshape([feature_shape[0], y_half_dim, x_half_dim, feature_shape[3]])(
                sym_end_block(quadrant)
            )
        )

    quadrants_corners_concatenated = Concatenate(axis=1)(
        [Lambda(lambda x: x[:, -1, -1, :])(q) for q in quadrants]
    )

    quadrants_corners_concatenated = Reshape(
        [quadrants_corners_concatenated.shape[1] * quadrants_corners_concatenated.shape[2]]
    )(quadrants_corners_concatenated)
